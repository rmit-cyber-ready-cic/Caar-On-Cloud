...
def lambda_handler(event, context):
    logger.info('<OrderFlowers_dev>> Lex event info = ' + json.dumps(event))
    ...
    logger.debug('<<OrderFlowers_dev> lambda_handler: session_attributes = ' + json.dumps(session_attributes))
    
    currentIntent = event.get('currentIntent', None)
    logger.info('currentIntent: ' + currentIntent["name"])
...
def make_appointment_intent_handler(intent_request, session_attributes):
    ...
    try:
        ...
        helpers.ses_code(slot_values['email'], slot_values['problem'])
...
def fallback_intent_handler(intent_request, session_attributes):
    ...
    logger.debug('<<OrderFlowers_dev>> fallback_intent_handler(): calling get_kendra_answer(query="%s")', query_string)
    ...
    if kendra_response is None:
        ...
        logger.debug('<<OrderFlowers_dev>> "fallback_intent_handler(): kendra_response = %s', kendra_response)
...
HANDLERS = {
    ...
    'kendraFallback':           {'handler': fallback_intent_handler}
}